  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-8">
      ...
      <motion.div variants={item}>
        <Card className="bg-gradient-to-br from-[#F9B000] via-[#E09E00] to-[#D09400] text-[#111111] border-0 overflow-hidden relative shadow-xl shadow-[#F9B000]/25">
          ...
          <CardContent className="p-8 lg:p-10 relative z-10">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div className="flex items-center gap-5">
                <div className="w-16 h-16 rounded-full bg-white/40 backdrop-blur-sm flex items-center justify-center text-2xl font-bold text-[#111111] border-2 border-white/60 shadow-lg">
                  {user?.firstName?.charAt(0) || 'U'}
                </div>
                <div>
                  <h2 className="text-[28px] font-bold text-[#111111] leading-tight" style={{ fontFamily: 'var(--font-display)' }}>
                    Welcome back, {user?.firstName || 'Partner'} 👋
                  </h2>
                  <div className="flex items-center gap-2 mt-1.5">
                    <p className="text-sm text-[#111111]/70 font-medium">Verified Shipping Partner</p>
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-3 px-5 py-3 rounded-[16px] bg-white/30 backdrop-blur-sm text-sm font-semibold shadow-sm border border-white/20">
                  ...
                  <div>
                    <span className="text-[#111111]/60 text-xs">Today's Target</span>
                    <div className="flex items-center gap-1.5">
                      <span className="font-bold text-[#111111]">{user?.todayDeliveries || DASHBOARD_STATS.todayDeliveries}</span>
                      <span className="text-[#111111]/50">/ 15 deliveries</span>
                    </div>
                  </div>
                </div>
                <div className="hidden sm:flex items-center gap-2 px-4 py-3 rounded-[16px] bg-white/30 backdrop-blur-sm border border-white/20">
                  ...
                  <span className="text-xs font-semibold text-[#111111]/80">Sector 14, Gurugram</span>
                </div>
              </div>
            </div>
            {/* Mini progress bar */}
            <div className="mt-6 flex items-center gap-4">
              ...
              <span className="text-xs font-semibold text-[#111111]/70 whitespace-nowrap">
      ...
      <motion.div variants={item} className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        {statCards(user).map((stat) => (
          <Card key={stat.label} className="hover:shadow-lg hover:border-[#F9B000]/20 transition-all duration-300 group overflow-hidden relative">
            ...
            <CardContent className="p-6 relative z-10">
              <div className="flex items-center justify-between mb-4">
                <div className={`w-12 h-12 rounded-[14px] bg-white/80 backdrop-blur-sm shadow-sm flex items-center justify-center group-hover:scale-110 transition-transform`}>
                  <stat.icon className={`h-6 w-6 ${stat.iconColor}`} />
                </div>
                <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">{stat.badge}</span>
              </div>
              <p className="text-[28px] font-bold text-[#111111] leading-tight" style={{ fontFamily: 'var(--font-display)' }}>{stat.value}</p>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <button className="text-xs font-semibold text-[#F9B000] hover:underline flex items-center gap-1">View All <ChevronRight className="h-3 w-3" /></button>
              </div>
            </CardHeader>
            <CardContent className="pt-2">
              <div className="space-y-2">
                {RECENT_ACTIVITY.map((activity: { id: number; action: string; time: string; type: string }) => {
                  ...
                  const color = activityColors[activity.type] || 'bg-gray-50 text-gray-500'
                  return (
                    <div key={activity.id} className="flex items-center gap-4 p-4 rounded-[14px] hover:bg-[#FAF8F5] transition-all group cursor-pointer">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400">4 of 5 verified</span>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-2">
              <div className="space-y-2">
                {documentItems.map((doc) => (
                  <div key={doc.label} className="flex items-center justify-between p-4 rounded-[14px] bg-[#FAF8F5] hover:bg-[#F0EDE8] transition-all group cursor-pointer">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-[12px] bg-white border border-[#E2DDD5] flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform`}>
                        <doc.icon className="h-4 w-4 text-gray-600" />
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-[#111111]">{doc.label}</p>
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <button key={action.label} className="group relative overflow-hidden rounded-[24px] p-5 border border-[#E2DDD5] hover:shadow-lg hover:border-[#F9B000]/30 transition-all duration-300 bg-white">
                  ...
                  <p className="text-sm font-bold text-[#111111]">{action.label}</p>
