...
type TabType = 'BUYERS' | 'SELLERS' | 'PARTNERS'
...
export function AdminProfile() {
  ...
  const tabs = [
    { id: 'BUYERS', label: 'Shoppers', icon: User, count: users.length, color: 'bg-primary' },
    { id: 'SELLERS', label: 'Artisans', icon: ShoppingBag, count: sellers.length, color: 'bg-accent' },
    { id: 'PARTNERS', label: 'Delivery', icon: Truck, count: partners.length, color: 'bg-saffron' }
  ] as const

  const getFilteredData = () => {
    let data: any[] = []
    if (activeTab === 'BUYERS') data = users
    if (activeTab === 'SELLERS') data = sellers
    if (activeTab === 'PARTNERS') data = partners
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-8 pb-10">
      ...
      <motion.div variants={item}>
        <Card className="bg-gradient-to-br from-forest to-forest/90 text-primary-foreground border-0 overflow-hidden relative shadow-xl">
          ...
          <CardContent className="p-8 lg:p-10 relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              <h2 className="text-[32px] font-bold leading-tight font-display mb-2">Master Directory</h2>
              <p className="text-white/80 font-medium">Manage all platform participants across UdrCrafts.</p>
            </div>
      ...
      {/* Tabs */}
      <motion.div variants={item} className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white ${tab.color} ${isActive ? 'opacity-100' : 'opacity-70'}`}>
                <tab.icon className="h-4 w-4" />
              </div>
              {tab.label}
              <span className={`text-xs px-2 py-1 rounded-full ${isActive ? 'bg-muted text-foreground' : 'bg-muted/50 text-muted-foreground'}`}>
                {tab.count}
              </span>
            </button>
          )
        ...
        <Card className="border-border">
          <CardHeader className="border-b border-border bg-muted/20">
            <CardTitle className="text-foreground">
              {activeTab === 'BUYERS' && 'Registered Shoppers'}
              {activeTab === 'SELLERS' && 'Approved Artisans'}
              {activeTab === 'PARTNERS' && 'Delivery Fleet'}
            </CardTitle>
          </CardHeader>
                  ...
                  {filteredData.map((person) => (
                    ...
                    >
                      <div className="flex items-center gap-5">
                        <div className="w-14 h-14 rounded-full bg-muted border border-border flex items-center justify-center text-xl font-bold text-muted-foreground shrink-0 overflow-hidden">
                          {person.profileImage ? (
                            ...
                            person.firstName?.charAt(0) || <User className="h-6 w-6" />
                          )}
                        </div>
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {activeTab === 'SELLERS' && (
                              <Badge variant={person.status === 'VERIFIED' ? 'success' : 'warning'} className="border-none">
                                {person.status}
                              </Badge>
                            )}
                            {activeTab === 'PARTNERS' && (
                              <Badge variant={person.status === 'Available' ? 'success' : 'secondary'} className="border-none">
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-muted-foreground">
                            ...
                            {activeTab === 'SELLERS' && person.businessName && (
                              <span className="flex items-center gap-1.5"><Building2 className="h-3.5 w-3.5" /> {person.businessName}</span>
                            )}
                            {activeTab === 'PARTNERS' && person.partnerId && (
                              <span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" /> ID: {person.partnerId}</span>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-6 text-sm">
                        {(activeTab === 'SELLERS' || activeTab === 'PARTNERS') && (
                          <div className="text-right flex items-center gap-4">
                            {person.status === 'PENDING' || person.status === 'Pending' ? (
                              <button 
                                onClick={async () => {
                                  try {
                            ...
                            <div>
                              <p className="text-muted-foreground text-xs uppercase font-bold tracking-wider mb-1">Rating</p>
                              <p className="font-bold flex items-center justify-end gap-1 text-foreground">
                                <Star className="h-4 w-4 fill-saffron text-saffron" /> {person.rating || '5.0'}
                              </p>
                            </div>
                          </div>
                        )}
                        <div className="text-right">
                          <p className="text-muted-foreground text-xs uppercase font-bold tracking-wider mb-1">Joined</p>
                          <p className="font-medium text-foreground flex items-center justify-end gap-1.5">
                            <Calendar className="h-3.5 w-3.5" /> {new Date(person.createdAt || person.dateJoined).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                    </motion.div>
