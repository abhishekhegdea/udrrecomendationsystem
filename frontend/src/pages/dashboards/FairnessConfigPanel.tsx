      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <label className="text-sm font-semibold text-foreground block truncate">
              {label}
            </label>
...
export function FairnessConfigPanel() {
  ...
  const fetchConfig =
    async () => {
      ...
      try {
        const response =
          await axios.get<FairnessConfig>(
            `${API_BASE}/fairness-config`,
            {
              signal:
                controller.signal,
        ...
        if (isNetwork) {
          console.warn(
            'FairnessConfig: recommendation service unreachable, using local defaults'
          )
        } else if (
          isTimeout
        ) {
          console.warn(
            `FairnessConfig: request timed out after ${FETCH_TIMEOUT_MS}ms, using local defaults`
          )
        }
      }
    }
  ...
  const saveConfig =
    async () => {
      ...
      try {
        const response =
          await axios.put<FairnessConfig>(
            `${API_BASE}/fairness-config`,
            {
              boost_amount:
                draft.boost_amount,
              ...
              new_seller_ratio:
                draft.new_seller_ratio,

              max_per_seller_ratio:
                draft.max_per_seller_ratio,
            },
            {
              timeout:
                FETCH_TIMEOUT_MS,
        ...
        toast.success(
          'Fairness configuration updated'
        )
      } catch (
        error: any
      ) {
        ...
        if (
          ...
          toast.error(
            'Recommendation service is offline — changes cannot be saved right now'
          )
        } else {
          toast.error(
            error?.response
              ...
              ?.detail?.[0]
              ?.msg ||
              'Failed to update fairness configuration'
          )
        }
      } finally {
        if (
              ...
              <div className="flex items-center justify-between pt-2">
                <div className="flex items-center gap-2">
                  {hasChanged && (
                    <Badge
                      variant="warning"
                      className="bg-amber-500/20 text-amber-700 border-none text-xs"
                    >
                      Unsaved
                      changes
                    </Badge>
